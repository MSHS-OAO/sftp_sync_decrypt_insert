...
# read all insert files that need jobcode check
...
# Check Paycode in new MSHQ data
...
mshq_paycodes <- mshq_paycodes %>%
         select(FILE_NAME, PAYCODE) %>%
    filter(!(PAYCODE %in% mapping_paycode)) %>%
    distinct()

if (nrow(mshq_paycodes) == 0 ) {
  print("MSHQ Paycodes are up to date in MAPPING_PAYCODE")
} else {
  kable(mshq_paycodes, 
        caption = "The following MSHQ paycodes need to be added to MAPING_PAYCODE:") %>% 
    kable_styling(latex_options = "striped", 
                  font_size = 10) %>%
    row_spec(0, font_size = 13)
}

# Check paycode in new BISLR data
...
bislr_paycodes <- bislr_paycodes %>%
         select(FILE_NAME, PAYCODE) %>%
    filter(!(PAYCODE %in% mapping_paycode)) %>%
    distinct()

if (nrow(bislr_paycodes) == 0 ) {
  print("BISLR Paycodes are up to date in MAPPING_PAYCODE")
} else {
  kable(bislr_paycodes, 
        caption = "The following BISLR paycodes need to be added to MAPING_PAYCODE:") %>% 
    kable_styling(latex_options = "striped", 
                  font_size = 10) %>%
    row_spec(0, font_size = 13)
}
...
new_mshq_paycycles <- mshq_paycycles %>%
   mutate(END_DATE= as.Date(END_DATE)) %>%
         select(FILE_NAME, END_DATE) %>%
    filter(!(END_DATE %in% unique_dates)) %>%
    distinct()

if (nrow(new_mshq_paycycles) == 0 ) {
  print("MSHQ End Dates are included in MAPPING_PAYCYCLE")
} else {
  kable(new_mshq_paycycles, 
        caption = "The following MSHQ End Dates are not included in MAPING_PAYCYCLES:") %>% 
    kable_styling(latex_options = "striped", 
                  font_size = 10) %>%
    row_spec(0, font_size = 13)
}

# Check paycycle in BISLR
...
new_bislr_paycycles <- bislr_paycycles  %>%
   mutate(END_DATE= as.Date(END_DATE)) %>%
         select(FILE_NAME, END_DATE) %>%
    filter(!(END_DATE %in% unique_dates)) %>%
    distinct()

if (nrow(new_bislr_paycycles ) == 0 ) {
  print("MSHQ End Dates are included in MAPPING_PAYCYCLE")
} else {
  kable(new_bislr_paycycles , 
        caption = "The following MSHQ End Dates are not included in MAPING_PAYCYCLES:") %>% 
    kable_styling(latex_options = "striped", 
                  font_size = 10) %>%
    row_spec(0, font_size = 13)
