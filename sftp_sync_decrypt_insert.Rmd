...
dsn <- "OAO Cloud DB Greg"
prod_dsn <- "OAO Cloud DB Staging"
...
mshq_db_files <- tbl(con, "LPM_DATA_MSHQ_ORACLE") %>%
  select(FILE_NAME) %>%
  distinct() %>%
  collect() %>%
  pull
...
bislr_db_files <- tbl(con, "LPM_DATA_BISLR_ORACLE") %>%
  select(FILE_NAME) %>%
  distinct() %>%
  collect() %>%
  pull()
...
jobcodes <- tbl(con, "LPM_MAPPING_JOBCODE") %>%
...
if (is.null(mshq_check)) {
  print("MSHQ Jobcodes are up to date in LPM_MAPPING_JOBCODE")
...
if (is.null(bislr_check)) {
  print("BISLR Jobcodes are up to date in LPM_MAPPING_JOBCODE")
...
mapping_paycode <- tbl(con, "LPM_MAPPING_PAYCODE") %>%
                   select(PAYCODE_RAW) %>%
                   distinct() %>%
                   collect() %>%
                   pull()
...
if(is.null(new_paycodes)) {
  print("MSHS Paycodes are up to date in LPM_MAPPING_PAYCODE")
...
unique_dates <- tbl(con, "LPM_MAPPING_PAYCYCLE") %>%
                    collect() %>%
...
if (is.null(new_paycycles)) {
  print("MSHS End Dates are included in LPM_MAPPING_PAYCYCLE")
...
jobcode_merge <- glue(
  "MERGE INTO LPM_MAPPING_JOBCODE destination
   USING LENANG01.LPM_MAPPING_JOBCODE source
...
paycode_merge <- glue(
  "MERGE INTO LPM_MAPPING_PAYCODE destination
   USING LENANG01.LPM_MAPPING_PAYCODE source
...
paycycle_merge <- glue(
  "MERGE INTO LPM_MAPPING_PAYCYCLE destination
   USING LENANG01.LPM_MAPPING_PAYCYCLE source
...
# merge statement for repdef mapping table
repdef_merge <- glue(
  "MERGE INTO LPM_MAPPING_REPDEF destination
   USING LENANG01.LPM_MAPPING_REPDEF source
   ON (destination.DEFINITION_CODE = source.DEFINITION_CODE AND
       destination.ORACLE_COST_CENTER = source.ORACLE_COST_CENTER AND
   ...
   UPDATE SET destination.DEFINITION_NAME = source.DEFINITION_NAME,
              ...
              destination.LEGACY_COST_CENTER = source.LEGACY_COST_CENTER,
              ...
              destination.FTE_TREND = source.FTE_TREND,
              destination.DEPARTMENT_BREAKDOWN = source.DEPARTMENT_BREAKDOWN
   WHEN NOT MATCHED THEN
   INSERT (destination.DEFINITION_NAME,
           ...
           destination.VP,
           destination.FTE_TREND,
           destination.DEPARTMENT_BREAKDOWN)
   VALUES (source.DEFINITION_NAME,
           source.KEY_VOLUME,
...
if (length(mshq_inserts_diff) > 0) {
  ...
  mshq_destination_schema <- "OAO_DEVELOPMENT"
...
if (length(bislr_inserts_diff) > 0) {
  ...
  bislr_destination_schema <- "OAO_DEVELOPMENT"
